...
export const newProduct = TryCatch(
  async (req: Request<{}, {}, NewProductRequestBody>, res, next) => {
    const { name, photos, category, stock, price, description } = req.body;
    ...
    if (photos.length > 5)
      return next(new ErrorHandler("You can only add upto 5 Photos", 400));

    if (!name || !photos || !category || !stock || !price || !description) {
      const ids = photos.map((photo: any) => photo.key);
...
export const updateProduct = TryCatch(async (req, res, next) => {
  ...
  if (photos && photos.length > 0) {
    const ids = product.photos.map((photo) => photo.key);
    await utapi.deleteFiles(ids);
